...
% basic parameters
model.x_dim= 7;   %dimension of state vector   (x, dx, y, dy, z, dz, turn rate)
...
model.B2= [ model.bt zeros(2,3); 
            zeros(2,1) model.bt zeros(2,2);
            zeros(2,2) model.bt zeros(2,1);
            zeros(1,3) model.T*model.sigma_turn ];     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
model.D= diag([ 2*(pi/180); 2*(pi/180); 10 ]);      %std for angle and range noise
...
model.m_birth(:,1)= [ -1500; 0; 250; 0; 0; 0; 0 ];
model.B_birth(:,:,1)= diag([ 50; 50; 50; 50; 50; 50; 6*(pi/180) ]);
model.P_birth(:,:,1)= model.B_birth(:,:,1)*model.B_birth(:,:,1)';
    
model.w_birth(2)= 2/100;                                              %birth term 2
model.m_birth(:,2)= [ -250; 0; 1000; 0; 0; 0; 0 ];
model.B_birth(:,:,2)= diag([ 50; 50; 50; 50; 50; 50; 6*(pi/180) ]);
model.P_birth(:,:,2)= model.B_birth(:,:,2)*model.B_birth(:,:,2)';

model.w_birth(3)= 3/100;                                              %birth term 3
model.m_birth(:,3)= [ 250; 0; 750; 0; 0; 0; 0]; 
model.B_birth(:,:,3)= diag([ 50; 50; 50; 50; 50; 50; 6*(pi/180) ]);
model.P_birth(:,:,3)= model.B_birth(:,:,3)*model.B_birth(:,:,3)';

model.w_birth(4)= 3/100;                                              %birth term 4
model.m_birth(:,4)= [ 1000; 0; 1500; 0; 0; 0; 0 ];
model.B_birth(:,:,4)= diag([ 50; 50; 50; 50; 50; 50; 6*(pi/180) ]);
