...
%filter parameters
filter.L_max= 50;                  %limit on number of Gaussians
filter.elim_threshold= 1e-6;        %pruning threshold
filter.merge_threshold= 4;          %merging threshold

filter.P_G= 0.9999;                           %gate size in percentage
...
%initial prior
w_update(1)= eps;                                   % number closest to 0
m_update(:,1)= [0.1;0;0.1;0;0.1;0];
P_update(:,:,1)= diag([1 1 1 1 1 1]).^2;   % cov
L_update = 1;
